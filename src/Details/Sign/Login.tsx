...
const Login = ({ onSwitch, onSuccess }: { onSwitch?: () => void, onSuccess?: () => void }) => {
  const { login } = useAuth();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (email && password) {
      ...
      if (success) {
        ...
        setError('User not found. Please sign up or check your credentials.');
        // Optionally redirect to signup automatically after a delay
        setTimeout(() => {
  ...
  return (
    <div style={{
      ...
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
        ...
        Log in to your caRya.krama account
      </motion.p>
